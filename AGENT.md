...
- **Token Feature & Key-Less PASETO Operations**: Feature-gate token logic behind `token = ["dep:rusty_paseto", "dep:macros"]`. Use compile-time constant `pub const KEY: [u8; 32] = macros::key!("PASETO_PASSWORD");` generated by `macros::key!` proc-macro so `tokenize()`, `encode_paseto()`, and `decode_paseto()` operate without expecting key parameters.
- **Token Bundle & Custom String Serialization**: `Bundle` wraps `access: Token<Access>` and `refresh: Token<Refresh>`. Manual `Serialize` outputs tokenized PASETO strings via `access.tokenize()?`. `Deserialize` (supported by `TokenVisitor` on `Token<T>`) automatically decrypts PASETO strings back to `Token<T>` using `KEY` during deserialization.
